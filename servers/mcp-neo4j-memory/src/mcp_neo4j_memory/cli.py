...
from .protocols import run_stdio_server, run_http_server, run_sse_server
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mode",
                        ...
                        help="Server mode: stdio (default), http or sse")
...
async def run_server(args: argparse.Namespace) -> None:
    ...
    if args.mode == "stdio":
        ...
        await run_http_server(memory, args.host, args.port)
    elif args.mode == "sse":
        # Use SSE protocol
        memory = await create_memory(args)
        await run_sse_server(memory, args.host, args.port)
    else:
        raise ValueError(f"Invalid mode: {args.mode}. Use 'stdio', 'http' or 'sse'")
