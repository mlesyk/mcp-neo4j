...
REM Default help
echo Usage: %0 [stdio^|http^|sse^|test^|all^|login]
echo.
echo Commands:
...
:http
call :publish_variant "http" "mcp-neo4j-memory:http" "false"
goto :summary
...
if "%1"=="all" (
    ...
    echo   %remote_base%:http
    echo   %remote_base%:%VERSION%-http
...
echo docker run -p 3001:3001 %remote_base%:http
echo.
