...
case "$1" in
    "stdio")
        ...
        ;;
    "http")
        publish_variant "http" "mcp-neo4j-memory:http" false
        ;;
    "sse")
    ...
    "login")
        ...
        ;;
    *)
        echo "Usage: $0 [--dry-run] [stdio|http|sse|test|all|login]"
        echo ""
        echo "Options:"
        ...
        echo "  http   - Publish HTTP REST API variant"
...
if [ "$1" = "all" ] || [ "$1" = "" ] || [ "$1" = "http" ]; then
    echo "  $REGISTRY/$NAMESPACE/$IMAGE_NAME:http"
    echo "  $REGISTRY/$NAMESPACE/$IMAGE_NAME:$VERSION-http"
fi
...
echo ""
echo "# Pull and run HTTP server:"
echo "docker run -p 3001:3001 $REGISTRY/$NAMESPACE/$IMAGE_NAME:http"
echo ""
