...
run_mcp_compliance() {
    ...
    docker-compose -f docker/docker-compose.mcp-compliance.yml up -d neo4j-test mcp-sse-server mcp-http-server
    ...
    for service in neo4j-test mcp-sse-server mcp-http-server; do
